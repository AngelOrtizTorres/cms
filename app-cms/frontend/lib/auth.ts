...
export async function login(
  ...
  try {
    const sessionResp = await apiPost<{ user: User; token?: string }>(
      "/session/login",
      { email, password },
    );
    const body = sessionResp as unknown as Record<string, unknown>;
    ...
    try {
      const apiResp = await apiPost<{ token?: string; user?: User }>(
        "/auth/login",
        { email, password },
      );
